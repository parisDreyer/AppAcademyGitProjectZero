...
class MetaCorgiSnacks
  ...
  def method_missing(name, *args)
    snack = "get_#{name}_info"
    taste = "get_#{name}_tastiness"
    info = @snack_box.send(snack, @box_id)
    tastiness = @snack_box.send(taste, @box_id)
    result = "Treat: #{info}: #{tastiness} "
    tastiness > 30 ? "* #{result}" : result
  end

#couldn't get the copy paste solution to work
